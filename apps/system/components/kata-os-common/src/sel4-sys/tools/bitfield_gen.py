...
c_typedef struct %(name)s %(name)s_t;"""

c_generator_template = \
...
class TaggedUnion:
    ...
    def rust_generate(self, params):
        ...
        # Generate tag enum
        print("#[repr(C)]")
        print("pub enum %sTag {" % self.name, file=output)
        if len(self.tags) > 0:
        ...
        # Generate tag reader
        templates = ([rust_tag_reader_entry_template] * (len(self.widths) - 1)
                   + [rust_tag_reader_final_template])

        fs = (rust_tag_reader_header_template % subs
            + "".join([template %
                         dict(subs,
                              mask=2 ** width - 1,
                              classmask=self.word_classmask(width),
                       ...
                       for template, width in zip(templates, self.widths)])
            + rust_tag_reader_footer_template % subs)

        emit_named("%s_get_%s" % (self.name, self.tagname), params, fs)

	# Generate tag eq reader

        templates = ([rust_tag_eq_reader_entry_template] * (len(self.widths) - 1)
                   + [rust_tag_eq_reader_final_template])

        fs = (rust_tag_eq_reader_header_template % subs
            + "".join([template %
                         dict(subs,
                              mask=2 ** width - 1,
                              classmask=self.word_classmask(width),
                              ...
                              shift=self.tag_offset[width] % self.base)
                       for template, width in zip(templates, self.widths)])
            + rust_tag_eq_reader_footer_template % subs)
            ...
            for field in ref.visible_order:
                ...
                if size < self.base:
                    ...
                    field_inits.append(
                        "           /* fail if user has passed bits that we will override */")
                    field_inits.append(
                        "           %s!((%s & !0x%x%s) == (if (0 != (%s & (1%s << %d))) {  0x%x } else { 0 }));" % (RUST_ASSERTS[options.environment], f_value, mask, suf, f_value, suf, self.base_bits - 1, high_bits))
                    field_inits.append(
    ...
    def c_generate(self, params):
        ...
        # Generate typedef
        print(c_typedef_template %
              {"type": C_TYPES[options.environment][self.base],
               "name": self.name,
               "multiple": self.multiple}, file=output)
        print(file=output)
        ...
        subs = {
            'inline': C_INLINE[options.environment],
            'union': self.name,
            'type':  C_TYPES[options.environment][self.union_base],
            'tagname': self.tagname,
            'suf': self.constant_suffix}

        # Generate tag reader
        templates = ([c_tag_reader_entry_template] * (len(self.widths) - 1)
                     + c_[tag_reader_final_template])

        fs = (c_tag_reader_header_template % subs
              + "".join([template %
                         dict(subs,
                              mask=2 ** width - 1,
                              classmask=self.word_classmask(width),
                              ...
                              shift=self.tag_offset[width] % self.base)
                         for template, width in zip(templates, self.widths)])
              + c_tag_reader_footer_template % subs)

        emit_named("%s_get_%s" % (self.name, self.tagname), params, fs)

        # Generate tag eq reader
        templates = ([c_tag_eq_reader_entry_template] * (len(self.widths) - 1)
                     + [c_tag_eq_reader_final_template])

        fs = (c_tag_eq_reader_header_template % subs
              + "".join([template %
                         dict(subs,
                              mask=2 ** width - 1,
                              classmask=self.word_classmask(width),
                              ...
                              shift=self.tag_offset[width] % self.base)
                         for template, width in zip(templates, self.widths)])
              + c_tag_eq_reader_footer_template % subs)

        emit_named("%s_%s_equals" % (self.name, self.tagname), params, fs)

        # Generate pointer lifted tag reader
        templates = ([c_ptr_tag_reader_entry_template] * (len(self.widths) - 1)
                     + [c_ptr_tag_reader_final_template])

        fs = (c_ptr_tag_reader_header_template % subs
              + "".join([template %
                         dict(subs,
                              mask=2 ** width - 1,
                              classmask=self.word_classmask(width),
                              ...
                              shift=self.tag_offset[width] % self.base)
                         for template, width in zip(templates, self.widths)])
              + c_ptr_tag_reader_footer_template % subs)
        ...
        for name, value, ref in self.tags:
            ...
            param_list = ["%s %s" % (C_TYPES[options.environment][self.base], field)
            ...
            for field in ref.visible_order:
                ...
                if field == self.tagname:
                    f_value = "(%s)%s_%s" % (C_TYPES[options.environment][self.base], self.name, name)
                else:
                    f_value = field
                ...
                if size < self.base:
                    ...
                    field_asserts.append(
                        ...
                        % (C_ASSERTS[options.environment], f_value, mask, suf, self.base_sign_extend,
            ...
            generator = c_union_generator_template % print_params
            ptr_generator = c_ptr_union_generator_template % print_params
            ...
            for field, offset, size, high in ref.fields:
                ...
                subs = {
                    ...
                    "block": ref.name,
                    "field": field,
                    "type": C_TYPES[options.environment][ref.base],
                    "assert": C_ASSERTS[options.environment],
                    "index": index,
                    "shift": shift,
                    "r_shift_op": read_shift,
                    "w_shift_op": write_shift,
                ...
                # Reader
                emit_named("%s_%s_get_%s" % (self.name, ref.name, field),
                           params, c_union_reader_template % subs)

                # Writer
                emit_named("%s_%s_set_%s" % (self.name, ref.name, field),
                           params, c_union_writer_template % subs)

                # Pointer lifted reader
                emit_named("%s_%s_ptr_get_%s" % (self.name, ref.name, field),
                           params, c_ptr_union_reader_template % subs)

                # Pointer lifted writer
                emit_named("%s_%s_ptr_set_%s" % (self.name, ref.name, field),
                           params, c_ptr_union_writer_template % subs)
...
class Block:
    ...
    def rust_generate(self, params):
        ...
        for field, offset, size, high in self.fields:
            ...
            if size < self.base:
                ...
                field_inits.append(
                    "        /* fail if user has passed bits that we will override */")
                field_inits.append(
                    "        %s((%s & !0x%x%s) == (if (0 != (%s & (1%s << %d))) { 0x%x } else { 0 }));" % (RUST_ASSERTS[options.environment], field, mask, suf, field, suf, self.base_bits - 1, high_bits))
                field_inits.append(
                    "        %s.words[%d] |= (%s & 0x%x%s) %s %d;" % \
        ...
        generator = rust_generator_template % \
            {"block":        self.name, \
             "args":         args, \
             "word_inits":   '\n'.join(word_inits), \
             "field_inits":  '\n'.join(field_inits)}

        ptr_generator = rust_ptr_generator_template % \
            {"block":        self.name, \
             "args":         ptr_args, \
             "word_inits":   '\n'.join(ptr_word_inits), \
        ...
        for field, offset, size, high in self.fields:
            ...
            # Reader
            emit_named("%s_get_%s" % (self.name, field), params,
                       rust_reader_template % subs)

            # Writer
            emit_named("%s_set_%s" % (self.name, field), params,
                       rust_writer_template % subs)
    ...
    def c_generate(self, params):
        ...
        for field, offset, size, high in self.fields:
            ...
            if size < self.base:
                ...
                field_asserts.append(
                    "    %s((%s & ~0x%x%s) == ((%d && (%s & (1%s << %d))) ? 0x%x : 0));"
                    % (C_ASSERTS[options.environment], field, mask, suf, self.base_sign_extend,
        ...
        generator = c_generator_template % print_params
        ptr_generator = c_ptr_generator_template % print_params
        ...
        for field, offset, size, high in self.fields:
            ...
            # Reader
            emit_named("%s_get_%s" % (self.name, field), params,
                       c_reader_template % subs)

            # Writer
            emit_named("%s_set_%s" % (self.name, field), params,
                       c_writer_template % subs)

            # Pointer lifted reader
            emit_named("%s_ptr_get_%s" % (self.name, field), params,
                       c_ptr_reader_template % subs)

            # Pointer lifted writer
            emit_named("%s_ptr_set_%s" % (self.name, field), params,
                       c_ptr_writer_template % subs)
...
if __name__ == '__main__':
    ...
    parser.add_option('--language', action='store', default='rust',  # XXX 'c'
                      choices=list(['c', 'rust']))
    parser.add_option('--environment', action='store', default='sel4',
                      choices=list(C_INCLUDES.keys()))
    ...
    if options.hol_defs:
        ...
        if options.language == 'c':
            ...
            print('\n'.join(map(lambda x: '#include <%s>' % x,
                                C_INCLUDES[options.environment])), file=out_file)
            for e in det_values(blocks, unions):
                e.c_generate(options)
            print("#endif", file=out_file)
        elif options.language == 'rust':
            for e in det_values(blocks, unions):
                e.rust_generate(options)
